    ...
    <el-container>
      ...
      <el-main>
        ...
        <el-breadcrumb separator="/" class="breadcrumb">
          <el-breadcrumb-item v-for="(item, index) in breadcrumb" :key="index" @click="navigateToPath(item.path)"
            :class="{ 'clickable': index < breadcrumb.length - 1 }">
            {{ item.name }}
          </el-breadcrumb-item>
        </el-breadcrumb>

        <el-table :data="fileList" style="width: 100%" @selection-change="handleSelectionChange" ref="fileTable"
          ...
          <el-table-column label="操作" width="150">
            <template #default="scope">
              <el-button size="small" @click="handleDownloadSingle(scope.row)" :disabled="scope.row.directory">
                下载
              </el-button>
              <el-button size="small" @click="handleOpenFolder(scope.row)" v-if="scope.row.directory">
                打开
              </el-button>
            </template>
          </el-table-column>
...
watch(currentPath, (newPath) => {
  updateBreadcrumbFromPath(newPath)
})

onMounted(() => {
  updateBreadcrumbFromPath(currentPath.value)
  loadFileList()
})
...
function navigateToPath(path) {
  ...
  updateBreadcrumbFromPath(path)
  loadFileList()
}

// 根据文件路径更新面包屑
function updateBreadcrumbFromPath(path) {
  const parts = path.split('/').filter(part => part)
  const breadcrumbItems = [
    { title: '首页', path: '/' },
    { title: '文件管理', path: '/file-manager' }
  ]
  ...
  parts.forEach(part => {
    ...
    breadcrumbItems.push({
      title: part,
      path: `/file-manager?path=${encodeURIComponent(currentPath)}`
    })
  })
